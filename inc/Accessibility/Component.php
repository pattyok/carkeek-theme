...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			'carkeek-theme-navigation',
			get_theme_file_uri( '/assets/js/navigation.min.js' ),
			array(),
			carkeek_theme()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'carkeek-theme-navigation', 'async', true );
		wp_script_add_data( 'carkeek-theme-navigation', 'precache', true );
		wp_localize_script(
			'carkeek-theme-navigation',
			'carkeekThemeScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'carkeek-theme' ),
				'collapse' => __( 'Collapse child menu', 'carkeek-theme' ),
			)
		);
	}
