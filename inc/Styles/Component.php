...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'carkeek-theme-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

		// Enqueue Typekit fonts if using.
		$typekit_fonts_url = $this->get_typekit_fonts_url();
		if ( ! empty( $typekit_fonts_url ) ) {
			wp_enqueue_style( 'carkeek-theme-typekit', $typekit_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = carkeek_theme()->get_asset_version( $css_dir . $data['file'] );
			$asset   = $css_uri . carkeek_theme()->get_asset_path( $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'carkeek-theme-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'carkeek-theme' ), $handle ) ), 'Carkeek Theme 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'carkeek_theme_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! carkeek_theme()->is_amp();
		 ...
		 */
		return apply_filters( 'carkeek_theme_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			'carkeek-theme-global'     => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'carkeek-theme-comments'   => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'carkeek-theme-content'    => array(
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			),
			'carkeek-theme-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return carkeek_theme()->is_primary_sidebar_active();
				},
			),
			'carkeek-theme-widgets'    => array(
				'file'             => 'widgets.min.css',
				'preload_callback' => function() {
					return carkeek_theme()->is_primary_sidebar_active();
				},
			),
			'carkeek-theme-front-page' => array(
		 ...
		 */
		$css_files = apply_filters( 'carkeek_theme_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'carkeek_theme_google_fonts', $google_fonts );
